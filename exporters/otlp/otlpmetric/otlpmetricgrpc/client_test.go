...
func TestNewExporter_collectorConnectionDiesThenReconnectsWhenInRestMode(t *testing.T) {
	...
	ctx := context.Background()
	exp := newGRPCExporter(t, ctx, mc.endpoint,
		otlpmetricgrpc.WithRetry(otlp.RetrySettings{Enabled: false}),
...
func TestNewExporter_WithTimeout(t *testing.T) {
	...
	for _, tt := range tts {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.Background()
			exp := newGRPCExporter(t, ctx, mc.endpoint, otlpmetricgrpc.WithTimeout(tt.timeout), otlpmetricgrpc.WithRetry(otlp.RetrySettings{Enabled: false}))
			defer func() {
				_ = exp.Shutdown(ctx)
			}()
