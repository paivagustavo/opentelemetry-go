...
func TestEndToEnd(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			opts: []otlptracehttp.Option{
				otlptracehttp.WithCompression(otlp.GzipCompression),
			},
		},
		{
		...
		},
		{
			...
			opts: []otlptracehttp.Option{
				otlptracehttp.WithMarshal(otlp.MarshalJSON),
			},
		},
	}
...
func TestUnreasonableBackoff(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 3*otlptracehttp.DefaultBackoff)
	defer cancel()
	exporter, err := otlptrace.NewExporter(ctx, driver)
	require.NoError(t, err)
	defer func() {
